...
function ClinicSite(): ReactElement {
  ...
  const submitBooking = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      await api.createBooking({
        ...
        animal_type: String(formData.get('animal_type') || ''), breed: String(formData.get('breed') || ''),
        preferred_date: String(formData.get('preferred_date') || ''), preferred_time: String(formData.get('preferred_time') || ''), notes: String(formData.get('notes') || ''),
      });
  ...
  return <main>
    ...
    <Testimonial />
    <section className="contact" id="contact"><div className="contact-copy"><p className="eyebrow">Come say hello</p><h2>Let’s look after<br/>them.</h2><div><p>No. 128 Enu-Ifite<br/>Awka, Anambra State</p><p className="phone-line"><PhoneIcon /><span><a href="tel:+2348039778902">0803 977 8902</a><br/><a href="tel:+2347003666535">0700 366 6535</a></span></p><p>Contact us to arrange your visit.</p></div></div><div className="contact-form"><p className="eyebrow">Request an appointment</p><form onSubmit={submitBooking}><label>Your name<input name="owner_name" autoComplete="name" aria-label="Your name" required /></label><label>Phone number<input name="phone" type="tel" autoComplete="tel" aria-label="Phone number" required /></label><label>Animal's name<input name="animal_name" aria-label="Animal's name" required /></label><label>Animal type<select name="animal_type" aria-label="Animal type" defaultValue="Dog" required><option>Dog</option><option>Cat</option><option>Livestock</option><option>Poultry</option><option>Other</option></select></label><label>Breed or description <small>(optional)</small><input name="breed" aria-label="Breed or description" placeholder="e.g. German Shepherd, goat, broiler" /></label><label>What do you need?<select name="service" aria-label="Visit type" defaultValue="General consultation"><option>General consultation</option><option>Wellness visit</option><option>Vaccination or preventive care</option><option>Grooming and wellness</option><option>Animal pharmacy / medicine advice</option><option>Farm or herd support</option><option>Emergency care</option></select></label><label>Preferred date <small>(optional)</small><input name="preferred_date" type="date" aria-label="Preferred visit date" /></label><label>Preferred time <small>(optional)</small><select name="preferred_time" aria-label="Preferred visit time" defaultValue=""><option value="">No preference</option><option>Morning (8am–12pm)</option><option>Afternoon (12pm–4pm)</option><option>Evening (4pm–6pm)</option></select></label><label className="booking-notes">What should we know? <small>(optional)</small><textarea name="notes" aria-label="Care notes" rows={3} maxLength={1000} placeholder="Symptoms, reason for visit, or anything that will help our team prepare." /></label><button type="submit">Send request <ArrowIcon /></button>{bookingMessage && <p className="booking-message" role="status">{bookingMessage}</p>}</form></div></section>
